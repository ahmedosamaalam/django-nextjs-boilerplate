...
import { LISTENER, MOD, SPEAKER } from '../constants/CONSTANTS';
...
export const CallProvider = ({ children }) => {
  ...
  const createRoom = async (roomName) => {
    ...
    try {
      const { data } = await POST(`/create-meeting-token`);
      return data?.success ? data.data : null;
    } catch (error) {
      setError(error.error);
      throw new Error(
        error.error?.info || error.error || 'Something went wrong.'
      );
    }
  };
  const getRoom = async (roomName) => {
    ...
    try {
      const { data } = await GET(`/room/${roomName}`);
      return data?.success ? data.data : null;
    } catch (error) {
      setError(error.error);
      throw new Error(
  ...
  const joinRoom = useCallback(
    async ({ userName, name, moderator }) => {
      ...
      const options = {
        ...
        url: `${process.env.REACT_APP_DAILY_DOMAIN}/${roomInfo?.name}`,
        userName,
      };
      if (roomInfo?.token) {
      ...
      if (newToken?.data?.token) {
        options.token = newToken?.data?.token;
      }
      ...
      function handleJoinedMeeting(evt) {
        ...
        setView(INCALL);
        // TODO: route base meeting
        // history.push(`${ClubHouseRoute}/${roomInfo?.name}`);
      ...
      await call
        .join(options)
        .then(async () => {
          await call.setUserData(userData);
          setError(false);
          setCallFrame(call);
  ...
  const leaveOrEndCall = useCallback(() => {
    if (!callFrame) return;
    async function leave() {
      await callFrame.leave();
    }

    const mc = participants.filter(
      (p) => getAccountType(p?.user_name) === MOD
    ).length;
  ...
  useEffect(() => {
    if (updateParticipants) {
      // console.log("[PARTICIPANT LIST]", callFrame?.participants());
